...
namespace Karbonite
{
    public class ORSX_ParticleExtractor : PartModule
    {
        [KSPField]
        ...
        private List<ORSX_ModuleParticleCollector> _extractors;
        ...
        private void FindExtractors()
        {
            if (vessel != null)
            {
                if (part.Modules.Contains("ORSX_ModuleParticleCollector"))
                {
                    _extractors = part.Modules.OfType<ORSX_ModuleParticleCollector>().ToList();
                }
            }
        }
